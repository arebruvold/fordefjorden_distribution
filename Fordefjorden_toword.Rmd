---
title: "Distribution of nanoparticles and trace elements in a Norwegian fjord"
output:
...
```{targets ctd_met, tar_simple = TRUE}

    ctd_met <-
      read_excel(path = '~/Documents/GitHub/fordefjorden_distribution/files_imgs/total_metals.xlsx', range = 'A11:BV47') %>%
      ...
      )) %>% slice(c(-1:-5, -8)) %>%  mutate(across(4:74,
                                                    ~ as.numeric(.x))) %>%
      filter(str_detect(Sample_ID, '^F[2|4]|LOD|Mean')) %>%
      rename(smpl_grp = Sample_ID) %>%
      pivot_longer(
        cols = c(4:74),
        names_to = 'isotope',
        values_to = 'Total_metal_ppb'
      ) %>%
      mutate(
        Total_metal_ug_l = as.numeric(Total_metal_ppb),
        Total_metal_ppb = NULL
      ) %>%
    ...
    #retrieve LOD to add to facets
    LOD_total_met <-
      ctd_met %>% filter(smpl_grp == 'LOD') %>% dplyr::select(isotope, Total_metal_ug_l) %>% filter(str_detect(isotope, 'Al|Mn|Fe|Pb|Ti|Si'))
    
    ctd_met <- ctd_met %>% # mean blank subtraction
      group_by(isotope) %>%
      mutate(Total_metal_ug_l = Total_metal_ug_l - Total_metal_ug_l[smpl_grp == 'Mean']) %>%
      filter(str_detect(isotope, 'Al|Mn|Fe|Pb|Ti|Si'))
...
tar_target(map_fordefjorden,{
  get_stamenmap(
    ...
    scale_x_continuous(breaks = seq(4.7, 6.2, 0.1),
                       limits = c(4.9, 6.0))
}),
...
ctd_met <- read_excel(path = '~/Documents/GitHub/fordefjorden_distribution/files_imgs/total_metals.xlsx', range ='A11:BV47') %>% 
  rename_with(
    ~ str_replace_all(.x,
                      pattern = c(
                        "(?<=[a-z])(?=[A-Z])|[\\-\\s(./]+" = "_",
                        "[\\[\\])]" = "")
    )) %>% slice(c(-1:-5,-8)) %>%  mutate(
  across(4:74,
         ~ as.numeric(.x))
) %>% {. ->> ctd_met_full} %>% 
  filter(str_detect(Sample_ID,'^F[2|4]|LOD|Mean')
  ...
  mutate(smpl_grp = str_replace_all(smpl_grp,'0','')
         ) %>%# add depth groupings
  mutate(
    depth = case_when(
      str_detect(smpl_grp,'\\-2') ~ 200,
...
#retrieve LOD to add to facets
LOD_total_met <- ctd_met %>% filter(smpl_grp == 'LOD') %>% dplyr::select(isotope, Total_metal_ug_l) %>% filter(str_detect(isotope,'Al|Mn|Fe|Pb|Ti|Si'))

ctd_met <- ctd_met %>% # mean blank subtraction
  group_by(isotope) %>% 
  mutate(
    Total_metal_ug_l = Total_metal_ug_l-Total_metal_ug_l[smpl_grp == 'Mean']
  )%>% 
  filter(str_detect(isotope,'Al|Mn|Fe|Pb|Ti|Si'))
...
tot_met_filter <- ctd_met %>% filter(station == "Inner" | station == "Outer",
  ...
  dplyr::select(!c("smpl_grp", "Lab_ID", "Unknown"))
...
ggsave(filename = "out/fig4-particle-mass-concentrations.tiff", width = 15, height = 8, device='tiff', dpi=300, units = "cm")
```
...
ggsave(filename = "out/fig6-mass-distributions.tiff", width = 15, height = 8, device = "tiff", dpi = 300, units = "cm")
...
ggsave(filename = "out/si-fig3-particle-number-concentrations.tiff", width = 15, height = 10, device='tiff', dpi=300, units = "cm")
...
validation_al <- validation %>% filter(isotope == "Ti")
